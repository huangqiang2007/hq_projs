...
typedef struct graph_desc {
    ...
    aipu_tensor_info_t break_dumps;  /**< struct contains break dump tensor descriptors info. */
    aipu_tensor_info_t printf_dumps; /**< struct contains printf dump data descriptors info. */
} aipu_graph_desc_t;
...
typedef struct aipu_buffer_alloc_info {
    ...
    aipu_tensor_buffer_t break_dumps;  /**< struct contains info. of break-dump tensor buffers allocated */
...
aipu_status_t AIPU_set_dump_options(const aipu_ctx_handle_t* ctx,
    uint32_t job_id, const aipu_dump_option_t* option);
/**
 ...
 *
 * @param[in] log_buffer_base Pointer to a buffer allocated by UMD stores the log information
 *
 * @retval AIPU_STATUS_SUCCESS
 * @retval AIPU_STATUS_ERROR_NULL_PTR
 *
 */
aipu_status_t AIPU_printf(char *log_buffer_base);
