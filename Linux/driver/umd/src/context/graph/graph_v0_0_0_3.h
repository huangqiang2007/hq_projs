...
/**
 * V3 & V4 share the same graph structure,
 * but some field interpretations are different.
 */
#define AIPU_LOADABLE_GRAPH_V0003     3
#define AIPU_LOADABLE_GRAPH_V0004     4

namespace AIRT
{
/**
 * @brief enum section type used to specify a memory region
 */
typedef enum {
    SECTION_TYPE_RODATA,
...
/**
 * @brief struct linked list descriptor
 */
typedef struct linked_list_desc {
    uint32_t node_cnt;
} linked_list_desc_t;

/**
 * @brief struct linked list node descriptor
 */
typedef struct list_node_desc {
    uint32_t type;
